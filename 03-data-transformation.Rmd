...
- Let's take a look at the `ipeds_sat` dataset.
...
- Please `filter()` the `ipeds_sat` dataset for rows where `percentile` is 25th, `sat_ebrw` is greater than or equal to 720, and `sat_math` is greater than or equal to 720. 
...
- Once again, first argument is dataframe.
...
- Using `summarize()`, instead of finding the mean and standard deviation with `mean()` and `sd()` , I'd like you to use the `min()` and `max()` functions to find the minimum and maximum values for `sat_math` in the `ipeds_sat` dataset. 
...
- Using `mutate`,  I'd like you to create a  new column in the `ipeds_sat` dataset that computes the average of `sat_ebrw` and `sat_math` for each row (i.e., `sat_ebrw` plus `sat_math` divided by 2). 
