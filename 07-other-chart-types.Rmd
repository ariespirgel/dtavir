...
- Let's create a summary of the data before plotting a line chart.
- Use `group_by()` and `summarize()` to find total headcount by `year` and `distance_type`.
- Now let's pipe that to a line chart with `year` on the x-axis, `total` on the y-axis, and have the color of the points be represented by `distance_type`.
